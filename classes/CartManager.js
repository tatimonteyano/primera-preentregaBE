...
class CartManager {
  ...
  addProductToCart(cartId, productId) {
    let cart_to_modify =
      this.carts[this.carts.findIndex((element) => element.id == cartId)];
    if (!cart_to_modify) {
      return "carrito inexistente";
    }
    if (
      !cart_to_modify.products.find((element) => element.product == productId)
    ) {
      cart_to_modify.products.push({ product: productId, quantity: 1 });
    } else {
      cart_to_modify.products[
        cart_to_modify.products.findIndex(
